...
from shore_tts.models.utils import apply_rotary_pos_emb, is_package_available
from shore_tts.utils.spectrogram import BN_MDCT_Spectrogram

# raw wav to BN-MDCT features

class MDCTSpec(nn.Module):
    def __init__(
        self,
        hop_length=441,
        n_bands=1,
    ...
    def forward(self, wav):
        ...
        if wav.ndim == 3:
            wav = wav.squeeze(1)
        if wav.ndim != 2:
            raise ValueError(f"Expected wav with shape (B, T) or (B, 1, T), got {tuple(wav.shape)}")

        feats = self.extractor(wav)  # (B, T, F)
        return feats.transpose(1, 2)  # (B, F, T)

    def inverse(self, feats: torch.Tensor, length: int | None = None) -> torch.Tensor:
        if feats.ndim != 3:
            raise ValueError(f"Expected feats with shape (B, F, T), got {tuple(feats.shape)}")
        return self.extractor.inverse(feats.transpose(1, 2), length=length)
...
class AttnProcessor:
    def __init__(
        ...
        attn_backend: str = "flash_attn",  # "torch" or "flash_attn"
