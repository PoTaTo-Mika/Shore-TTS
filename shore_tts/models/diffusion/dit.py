# This file is from https://github.com/SWivid/F5-TTS/blob/main/src/f5_tts/model/modules.py
# Partly modified.
...
from shore_tts.models.diffusion.modules import (
    AdaLayerNorm_Final,
    ConvNeXtV2Block,
...
from shore_tts.models.utils import RotaryEmbedding
...
class TextEmbedding(nn.Module):
    def __init__(
        ...
        if conv_layers > 0:
            ...
            self.precompute_max_pos = 16384  # 16384 is about 66s of MDCT based audio.
    ...
    def forward(self, text: int["b nt"], seq_len, drop_text=False):
        # Embedding indices must stay integral even if an upstream path hands us float tensors.
        ...
        text = text[:, :max_seq_len]  # curtail if character tokens are more than the feature frames
...
class InputEmbedding(nn.Module):
    def __init__(self, spec_dim, text_dim, out_dim):
        super().__init__()
        self.proj = nn.Linear(spec_dim * 2 + text_dim, out_dim)
...
class DiT(nn.Module):
    def __init__(
        ...
        *,
        ...
        if text_num_embeds is None:
            raise ValueError("DiT requires `text_num_embeds`; Shore-TTS no longer uses a fixed byte vocabulary size.")
        ...
        self.input_embed = InputEmbedding(spec_dim, text_dim, dim)
        ...
        self.proj_out = nn.Linear(dim, spec_dim)
    ...
    def forward(
        self,
        x: float["b n d"],  # noised input features
        cond: float["b n d"],  # masked conditioning features
        text: int["b nt"],  # text
        ...
        # t: conditioning time, text: text tokens, x: noised features + cond features + text
