...
class CFM(nn.Module):
    def __init__(
        ...
        cond_drop_prob=0.2,
        num_channels=None,
        spec_module: nn.Module | None = None,
        ...
        # Spectrum extractor.
        spec_kwargs = default(spec_kwargs, {})
    ...
    def tokenize_text(self, text: list[str] | list[list[str]], device: torch.device) -> torch.Tensor:
        ...
        if not text:
            return list_str_to_idx([], self.vocab_char_map).to(device)

        if isinstance(text[0], str):
            if self.text_tokenizer is None:
                raise ValueError("CFM requires `text_tokenizer` to convert raw strings into pinyin tokens.")
            text = [self.text_tokenizer.text_to_pinyin(item) for item in text]
    ...
    def sample(
        ...
        *,
        ...
        if isinstance(text, list):
            text = self.tokenize_text(text, device)
    ...
    def forward(
        self,
        inp: float["b n d"] | float["b nw"],  # MDCT features or raw wave
        text: int["b nt"] | list[str],
        *,
        lens: int["b"] | None = None,
