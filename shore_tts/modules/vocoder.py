...
from shore_tts.audio_process.wav2mel import LogMelSpectrogram
...
if __name__ == "__main__":
    ...
    device='cuda:0'
    # 模型路径
    model_path = 'checkpoints/vocoder/model.safetensors'
    # 模型实例化
    model = ADaMoSHiFiGANV1(model_path).to('cpu')
    # 模型加载到指定设备
    model.to(device)
    # 测试音频加载
    wav, sr = librosa.load(audio, sr=44100, mono=True)
    # 测试音频转换为tensor，因为模型输入为tensor
    wav = torch.from_numpy(wav).float()[None].to(device)
    # 测试音频编码为mel
    mel = model.encode(wav)
    print(mel.shape) 
    # [1, 128, [(audio_length * sample_rate - n_fft) / hop_length + 1]]
    # 测试音频解码为wav
    wav = model.decode(mel)[0].mT
    # 测试音频保存
    sf.write("rebuild.wav", wav.cpu().numpy(), 44100)